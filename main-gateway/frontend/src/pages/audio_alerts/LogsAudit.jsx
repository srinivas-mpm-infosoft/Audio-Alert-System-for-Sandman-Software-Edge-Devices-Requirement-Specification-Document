...
function DestIps({ value }) {
  ...
  return (
    <div className="flex flex-col gap-0.5">
      {ips.map((ip, i) => (
        <span key={i} className="font-mono text-[11px] text-slate-600">{ip}</span>
      ))}
    </div>
  );
}
...
function SortHeader({ label, sortKey, sortState, onSort }) {
  ...
  return (
    <th
      className="px-3 py-3 text-left font-semibold text-slate-600 text-[10px] uppercase tracking-wide whitespace-nowrap cursor-pointer select-none hover:bg-slate-100/60 transition-colors"
            ...
            <div className="flex flex-wrap gap-2 items-center justify-between">
              <form onSubmit={handleSearchSubmit} className="flex gap-2 flex-wrap items-center">
                <div className="relative">
                  ...
                  <input
                    className="pl-8 pr-3 py-1.5 border border-slate-200 rounded-lg text-xs bg-white focus:outline-none focus:ring-1 focus:ring-indigo-400 w-52"
                ...
                <table className="w-full text-sm" role="table">
                  <thead>
                    <tr className="border-b border-slate-100 bg-slate-50/60">
                      ...
                      <th className="px-3 py-3 text-left font-semibold text-slate-600 text-[10px] uppercase tracking-wide">Destination IPs</th>
                    ...
                    {alertLogs.map((row, idx) => {
                      ...
                      return (
                        <tr key={row.id ?? idx} className="hover:bg-indigo-50/20 transition-colors">
                          ...
                          <td className="px-3 py-3 text-xs font-medium text-slate-800 max-w-[200px]">
                          ...
                          <td className="px-3 py-3 text-xs text-slate-500">{zone || "—"}</td>
                          <td className="px-3 py-3"><DestIps value={destIps} /></td>
                          <td className="px-3 py-3">
                            {status ? (
                              <span className={`inline-block text-[10px] font-bold px-2 py-0.5 rounded-full ${DELIVERY_COLORS[statusKey] ?? "bg-slate-100 text-slate-600"}`}>
                                {status}
                              </span>
                            ) : <span className="text-slate-400 text-xs">—</span>}
                          </td>
          ...
          <div className="p-4 flex flex-col gap-4">
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div className="flex gap-2 flex-wrap">
                ...
                <input
                  className="border border-slate-200 rounded-lg px-2 py-1.5 text-xs bg-white focus:outline-none focus:ring-1 focus:ring-indigo-400 w-32 text-slate-600"
