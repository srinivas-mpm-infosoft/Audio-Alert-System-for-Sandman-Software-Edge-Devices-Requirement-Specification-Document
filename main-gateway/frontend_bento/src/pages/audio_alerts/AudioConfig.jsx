...
import { Upload, Trash2, Plus, Loader2, Volume2, Pencil, Mic } from "lucide-react";
...
import { Button, IconButton, Table, td } from "../../components/ui/Bento";
...
export default function AudioConfig() {
  ...
  const [addMode, setAddMode] = useState("file"); // "file" | "record" — how the Upload Clip modal captures audio
    ...
    <div className="flex flex-col gap-5">
          ...
          <div className="flex overflow-x-auto" role="tablist">
            {TABS.map((t, i) => (
              <button key={i} role="tab" aria-selected={tab === i} onClick={() => setTab(i)}
                className={`px-5 py-3 text-sm font-semibold whitespace-nowrap transition-colors focus:outline-none ${tab === i ? "border-b-2 border-emerald-700 text-emerald-800" : "text-gray-400 hover:text-gray-600"}`}>
                {t}
              </button>
            ))}
          </div>
        ...
        <div className="p-5">
            ...
            <div className="flex flex-col gap-4">
              {canUpload && (
                <div className="flex justify-end">
                  <Button variant="primary" icon={Upload} onClick={() => setUploadOpen(true)}>Upload Clip</Button>
                </div>
              )}
              {loading ? <div className="flex justify-center py-12"><Loader2 className="h-7 w-7 animate-spin text-emerald-700" /></div> : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {clips.map((clip) => (
                    <div key={clip.id} className="border border-gray-100 rounded-2xl p-4 flex flex-col gap-2 hover:shadow-md transition-shadow">
                      <div className="flex items-start justify-between">
                        <div className="min-w-0">
                          <p className="font-semibold text-gray-900 text-sm truncate">{clip.name}</p>
                          {clip.alert_code && <p className="text-xs text-emerald-700 font-mono">{clip.alert_code}</p>}
                        </div>
                      ...
                      {clip.file_path && (
                        <p className="text-[10px] text-gray-300 font-mono truncate" title={clip.file_path}>{clip.file_path}</p>
                      )}
                      ...
                      <div className="flex items-center gap-2 pt-1 border-t border-gray-100">
                        ...
                        {canUpload && (
                          <IconButton icon={Pencil} label="Edit clip" className="ml-auto" onClick={() => { setEditingClip({ ...clip }); setEditSelectedFile(null); }} />
                        )}
                        {canDelete && (
                          <IconButton icon={Trash2} danger label="Delete clip" className={!canUpload ? "ml-auto" : ""} onClick={() => { setDeleteTarget(clip.id); setDeleteType("clip"); }} />
                        )}
                      </div>
                    </div>
                  ))}
                  {clips.length === 0 && <div className="col-span-3"><EmptyState title="No clips uploaded" message="Upload or record a clip to use in manual broadcasts, schedules, and SOP steps." /></div>}
                </div>
              )}
            </div>
          )}
          ...
          {tab === 1 && (
            <div className="flex flex-col gap-4">
              ...
              <Table head={["Zone", "Audio Language"]}>
                {zones.map((zone) => (
                  <tr key={zone.id} className={zonesDirty[zone.id] ? "bg-amber-50/40" : ""}>
                    <td className={`${td} font-medium text-gray-900`}>{zone.name}</td>
                    <td className={td}>
                      <select
                        value={zone.default_language ?? "EN"}
                        onChange={(e) => handleZoneFieldChange(zone.id, "default_language", e.target.value)}
                        className="border border-gray-200 rounded-lg px-2 py-1 text-xs bg-white focus:outline-none focus:ring-1 focus:ring-emerald-500 text-gray-700"
                      >
              ...
              <div className="flex justify-end">
                <Button variant="primary" loading={zoneSaving} onClick={handleSaveZoneLanguages}>
                  {zoneSaving ? "Saving…" : "Save Zone Languages"}
                </Button>
              </div>
            </div>
          )}
            ...
            <div className="flex flex-col gap-6">
              <div>
                ...
                <div className="flex items-center gap-4">
                  <input id="master-vol" type="range" min={0} max={100} value={masterVolume} onChange={(e) => setMasterVolume(+e.target.value)} className="flex-1 accent-emerald-700" />
                  <span className="w-12 text-right font-bold text-gray-700">{masterVolume}%</span>
                </div>
              </div>
              <div>
                ...
                <div className="space-y-3">
                  {PRIORITIES.map((p) => (
                    <div key={p} className="flex items-center gap-4">
                      <span className="w-20 text-sm font-medium text-gray-700">{p}</span>
                      <input type="range" min={-12} max={12} value={priorityOffsets[p] ?? 0}
                        onChange={(e) => setPriorityOffset(p, +e.target.value)} className="flex-1 accent-emerald-700" />
                      <span className="w-14 text-right font-bold text-gray-700 font-mono">{priorityOffsets[p] >= 0 ? "+" : ""}{priorityOffsets[p] ?? 0} %</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="flex justify-end pt-2 border-t border-gray-100">
                <Button variant="primary" icon={Volume2} loading={audioSaving} onClick={handleSaveVolume}>
      ...
      {uploadOpen && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center">
          <div className="absolute inset-0 bg-black/40" onClick={() => { setUploadOpen(false); setSelectedFile(null); setAddMode("file"); }} />
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6 z-10 flex flex-col gap-4">
            <h3 className="font-bold text-gray-900">Add Audio Clip</h3>
            ...
            <div>
              ...
              <div className="inline-flex rounded-xl border border-gray-200 p-0.5 bg-gray-50 mb-3">
                {[["file", "Upload file"], ["record", "Record voice"]].map(([mode, label]) => (
                  <button key={mode} type="button" onClick={() => { setAddMode(mode); setSelectedFile(null); }}
                    className={`px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-colors ${addMode === mode ? "bg-white text-emerald-800 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}>
                    {label}
                  </button>
                ))}
              </div>
              ...
              {addMode === "file" ? (
                <>
                  <div onClick={() => fileRef.current?.click()}
                    className="border-2 border-dashed border-gray-200 rounded-xl p-5 text-center text-gray-400 text-sm cursor-pointer hover:border-emerald-300 transition-colors">
                    <Upload size={20} className="mx-auto mb-2 opacity-50" />
                    {selectedFile
                      ? <p className="text-gray-700 font-medium">{selectedFile.name} <span className="text-xs text-gray-400">({formatFileSize(selectedFile.size)})</span></p>
                ...
                <VoiceRecorder onCapture={setSelectedFile} />
              )}
            </div>

            <div className="flex gap-2 justify-end">
              <button type="button" onClick={() => { setUploadOpen(false); setSelectedFile(null); setAddMode("file"); }} className="px-3.5 py-1.5 border border-gray-200 rounded-full text-sm text-gray-600 hover:bg-gray-50">Cancel</button>
              <Button variant="primary" onClick={handleUploadClip} disabled={!newClip.name || (addMode === "record" && !selectedFile)}>
                {addMode === "record" ? "Save Clip" : "Upload"}
              </Button>
            </div>
          </div>
        </div>
      ...
      {editingClip && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6 z-10 flex flex-col gap-4">
            <h3 className="font-bold text-gray-900">Edit Audio Clip</h3>
            ...
            <div>
              ...
              {editingClip.file_path && !editSelectedFile && (
                <p className="text-[11px] text-gray-400 font-mono mb-1 truncate" title={editingClip.file_path}>
                  Current: {editingClip.file_path.split("/").pop()}
                </p>
              )}
              <div onClick={() => editFileRef.current?.click()}
                className="border-2 border-dashed border-gray-200 rounded-xl p-4 text-center text-gray-400 text-sm cursor-pointer hover:border-emerald-300 transition-colors">
                <Upload size={16} className="mx-auto mb-1 opacity-50" />
                {editSelectedFile
                  ? <p className="text-gray-700 font-medium text-xs">{editSelectedFile.name} ({formatFileSize(editSelectedFile.size)})</p>
            ...
            <div className="flex gap-2 justify-end">
              <button type="button" onClick={() => { setEditingClip(null); setEditSelectedFile(null); }} className="px-3.5 py-1.5 border border-gray-200 rounded-full text-sm text-gray-600 hover:bg-gray-50">Cancel</button>
              <Button variant="primary" onClick={handleUpdateClip} disabled={!editingClip.name}>Save Changes</Button>
            </div>
          </div>
        </div>
      )}
