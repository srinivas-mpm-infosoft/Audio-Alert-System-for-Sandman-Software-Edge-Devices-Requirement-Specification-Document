...
function StepEditor({ step, index, total, clips, onChange, onRemove, onMoveUp, onMoveDown, onUpload, uploading }) {
  ...
  return (
    <div className="border border-gray-200 rounded-xl p-4 flex flex-col gap-3 bg-gray-50/40">
      <div className="flex items-center gap-2">
        <GripVertical size={14} className="text-gray-300" aria-hidden="true" />
        <span className="text-xs font-bold text-gray-400 uppercase tracking-wide">Step {index + 1}</span>
        <div className="ml-auto flex items-center gap-1">
          <button type="button" onClick={onMoveUp} disabled={index === 0} className="p-1 rounded text-gray-400 hover:text-emerald-800 hover:bg-emerald-50 disabled:opacity-30 transition-colors" aria-label="Move step up">
            <ChevronUp size={14} />
          </button>
          <button type="button" onClick={onMoveDown} disabled={index === total - 1} className="p-1 rounded text-gray-400 hover:text-emerald-800 hover:bg-emerald-50 disabled:opacity-30 transition-colors" aria-label="Move step down">
            <ChevronDown size={14} />
          </button>
      ...
      <div className="flex gap-3">
        {[{ v: "text", l: "Dynamic Text" }, { v: "clip", l: "Pre-Recorded Voice" }].map(({ v, l }) => (
          <label key={v} className="flex items-center gap-2 cursor-pointer">
            <input type="radio" name={`step-mode-${index}`} value={v} checked={step.audio_mode === v} onChange={() => set("audio_mode", v)} className="text-emerald-800" />
        ...
        <div className="flex flex-col gap-2">
          ...
          <div>
            <button type="button" onClick={() => fileRef.current?.click()} disabled={uploading}
              className="inline-flex items-center gap-1.5 text-xs font-medium text-emerald-800 hover:text-emerald-900 disabled:opacity-50">
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-3">
        <button type="button" onClick={onCancel} className="p-2 rounded-lg hover:bg-gray-100 text-gray-500" aria-label="Back to SOPs">
          <ArrowLeft size={16} aria-hidden="true" />
        </button>
        <h2 className="text-lg font-bold text-gray-800">{initialSop ? "Edit SOP" : "New SOP"}</h2>
      </div>
      ...
      <div className={SECTION}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <p className="text-[10px] text-gray-400 mt-1">If a step isn't acknowledged within this time, it replays and the retry count increases. It never auto-advances.</p>
          </div>
          <label className="flex items-center gap-2 cursor-pointer self-end pb-2">
            <input type="checkbox" checked={form.is_active} onChange={(e) => set("is_active", e.target.checked)} className="rounded border-gray-300 text-emerald-800" />
            <span className="text-sm text-gray-700">Active (can be started from the dashboard)</span>
          </label>
        </div>
      </div>
      ...
      <div className={SECTION}>
        ...
        <label className="flex items-center gap-2 cursor-pointer mb-1">
          <input type="checkbox" checked={form.plant_wide} onChange={(e) => set("plant_wide", e.target.checked)} className="rounded border-gray-300 text-emerald-800" />
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={addStep} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-800 hover:bg-emerald-800 text-white rounded-lg text-xs font-semibold">
      ...
      <div className="flex justify-end gap-3">
        <button type="button" onClick={onCancel} className="px-4 py-2 border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50">
          Cancel
        </button>
        <button type="button" onClick={handleSubmit} disabled={saving}
          className="inline-flex items-center gap-2 px-5 py-2 bg-emerald-800 hover:bg-emerald-800 text-white rounded-lg text-sm font-semibold disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:ring-offset-1">
