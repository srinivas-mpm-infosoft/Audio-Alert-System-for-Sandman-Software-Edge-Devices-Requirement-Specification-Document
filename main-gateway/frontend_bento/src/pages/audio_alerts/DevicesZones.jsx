  ...
  return (
    <span className="flex items-center gap-1">
      <input autoFocus value={v} onChange={(e) => setV(e.target.value)}
        ...
        className="border border-emerald-300 rounded px-2 py-0.5 text-xs w-36 focus:outline-none focus:ring-1 focus:ring-emerald-500" />
      <button type="button" onClick={() => onSave(v)} className="text-emerald-600 hover:text-emerald-700"><Check size={13} /></button>
      <button type="button" onClick={onCancel} className="text-gray-400 hover:text-red-500"><X size={13} /></button>
    </span>
  );
}

function DeviceTypeIcon({ type }) {
  if (type === "Edge Node") return <span className="text-emerald-800 font-bold text-xs">EN</span>;
  return <span className="text-gray-500 font-bold text-xs">{(type || "DEV").slice(0, 3).toUpperCase()}</span>;
}
...
function LanguageSettings({ plants, zones, languages, showToast, canEdit }) {
  ...
  const LangSelect = ({ value, onChange }) => (
    <select
      ...
      disabled={!canEdit}
      className="border border-gray-200 rounded-lg px-2 py-1.5 text-xs bg-white focus:outline-none focus:ring-1 focus:ring-emerald-500 text-gray-700 disabled:bg-gray-50 disabled:text-gray-400"
    >
      {languages.map((l) => <option key={l.code} value={l.code}>{l.code} — {l.label}</option>)}
    </select>
  );

  if (loading) return <div className="flex justify-center py-12"><Loader2 className="h-6 w-6 animate-spin text-emerald-800" /></div>;
    ...
    <div className="flex flex-col gap-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {LANG_TYPES.map((t) => (
            <button key={t.key} type="button"
              ...
              className={`text-left px-4 py-3 rounded-xl border transition-colors ${activeType === t.key ? "border-emerald-500 bg-emerald-50" : "border-gray-200 bg-white hover:border-gray-300"} ${!canEdit ? "cursor-default" : "cursor-pointer"}`}>
              <p className={`text-xs font-bold mb-0.5 ${activeType === t.key ? "text-emerald-800" : "text-gray-700"}`}>{t.label}</p>
      ...
      {activeType === "plant" && (
        <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
          ...
          {plants.length === 0 && <p className="text-xs text-gray-400 italic">No plants configured.</p>}
          {plants.map((p) => (
            <div key={p.id} className="flex items-center justify-between gap-4">
              <span className="text-sm font-medium text-gray-700 flex-1">{p.name} <span className="text-xs text-gray-400">({p.location})</span></span>
      ...
      {activeType === "zone" && (
        <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-2">
          ...
          {zones.length === 0 && <p className="text-xs text-gray-400 italic">No zones configured.</p>}
            ...
            {zones.map((z) => (
              <div key={z.id} className="flex items-center justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <span className="ml-1.5 text-[11px] text-gray-400">{z.type}</span>
                </div>
      ...
      {activeType === "shift" && (
        <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
          ...
          <p className="text-xs text-gray-400">The selected language will be applied to all zones during that shift.</p>
          {SHIFTS.map((s) => (
            <div key={s} className="flex items-center justify-between gap-4">
              <span className="text-sm font-medium text-gray-700 w-28">{s}</span>
      ...
      {canEdit && (
        <div className="flex justify-end">
          <button type="button" onClick={handleSave} disabled={saving}
            className="inline-flex items-center gap-2 px-5 py-2 bg-emerald-800 hover:bg-emerald-800 text-white rounded-lg text-sm font-semibold disabled:opacity-50 transition-colors">
...
function PlantManagement({ showToast }) {
  ...
  if (loading) return <div className="flex justify-center py-16"><Loader2 className="h-7 w-7 animate-spin text-emerald-800" /></div>;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-2 border-b border-gray-200 pb-3">
        {SECTIONS.map((s) => (
          <button key={s.key} onClick={() => { setActiveSection(s.key); setAddOpen(false); }}
            className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-colors ${activeSection === s.key ? "bg-emerald-800 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
            {s.label} <span className="ml-1 opacity-60">({s.count})</span>
          </button>
        ))}
        <div className="ml-auto flex gap-2">
          <button type="button" onClick={reload} className="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 transition-colors"><RefreshCw size={14} /></button>
          {canEdit && (
            <button type="button" onClick={() => setAddOpen((o) => !o)}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-800 text-white rounded-lg text-xs font-semibold hover:bg-emerald-800">
      ...
      {/* Add form */}
      {addOpen && canEdit && (
        <div className="border border-emerald-200 rounded-xl p-4 bg-emerald-50 flex flex-col gap-3">
          {activeSection === "plants" && (
            <>
              <h4 className="text-sm font-semibold text-gray-800">New Plant</h4>
              ...
              <div className="flex gap-2 justify-end">
                <button onClick={() => setAddOpen(false)} className="px-3 py-1.5 border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-50">Cancel</button>
                <button onClick={handleAddPlant} disabled={!newPlant.name.trim()} className="px-3 py-1.5 bg-emerald-800 text-white rounded-lg text-sm font-semibold disabled:opacity-50">Add Plant</button>
              </div>
            </>
          )}
          {activeSection === "lines" && (
            <>
              <h4 className="text-sm font-semibold text-gray-800">New Line</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
              ...
              <div className="flex gap-2 justify-end">
                <button onClick={() => setAddOpen(false)} className="px-3 py-1.5 border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-50">Cancel</button>
                <button onClick={handleAddLine} disabled={!newLine.name.trim() || !newLine.plant_id} className="px-3 py-1.5 bg-emerald-800 text-white rounded-lg text-sm font-semibold disabled:opacity-50">Add Line</button>
              </div>
            </>
          )}
          {activeSection === "zones" && (
            <>
              <h4 className="text-sm font-semibold text-gray-800">New Zone</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
              ...
              <div className="flex gap-2 justify-end">
                <button onClick={() => setAddOpen(false)} className="px-3 py-1.5 border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-50">Cancel</button>
                <button onClick={handleAddZone} disabled={!newZone.name.trim() || !newZone.line_id} className="px-3 py-1.5 bg-emerald-800 text-white rounded-lg text-sm font-semibold disabled:opacity-50">Add Zone</button>
              </div>
            </>
          )}
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-x-auto">
        {activeSection === "plants" && (
          <table className="w-full text-sm">
            <thead><tr className="border-b border-gray-100 bg-gray-50/60">
              {["Plant Name", "Location", "Lines", ""].map((h) => <th key={h} className="px-4 py-3 text-left text-[11px] font-semibold text-gray-600 uppercase tracking-wide">{h}</th>)}
            </tr></thead>
            <tbody className="divide-y divide-gray-50">
              {plants.map((p) => (
                <tr key={p.id} className="hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-3 text-xs text-gray-500">{lines.filter((l) => l.plant_id === p.id).length}</td>
                  <td className="px-4 py-3">
                    {canEdit && (
                      <div className="flex gap-1 justify-end">
                        <button onClick={() => { setEditId(p.id); setEditField("name"); }} className="p-1.5 rounded text-gray-400 hover:text-emerald-800 hover:bg-emerald-50" title="Edit name"><Pencil size={12} /></button>
                        <button onClick={() => { setEditId(p.id); setEditField("location"); }} className="p-1.5 rounded text-gray-400 hover:text-emerald-800 hover:bg-emerald-50" title="Edit location"><MapPin size={12} /></button>
                        <button onClick={() => setDelTarget({ id: p.id, type: "plant", name: p.name })} className="p-1.5 rounded text-gray-400 hover:text-red-500 hover:bg-red-50" title="Delete"><Trash2 size={12} /></button>
                      </div>
                    )}
                  </td>
                </tr>
        ...
        {activeSection === "lines" && (
          <table className="w-full text-sm">
            <thead><tr className="border-b border-gray-100 bg-gray-50/60">
              {["Line Name", "Plant", "Zones", ""].map((h) => <th key={h} className="px-4 py-3 text-left text-[11px] font-semibold text-gray-600 uppercase tracking-wide">{h}</th>)}
            </tr></thead>
            <tbody className="divide-y divide-gray-50">
              {lines.map((l) => (
                <tr key={l.id} className="hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-3 text-xs text-gray-500">{plants.find((p) => p.id === l.plant_id)?.name || l.plant_id}</td>
                  <td className="px-4 py-3 text-xs text-gray-500">{zones.filter((z) => z.line_id === l.id).length}</td>
                  <td className="px-4 py-3">
                    {canEdit && (
                      <div className="flex gap-1 justify-end">
                        <button onClick={() => { setEditId(l.id); setEditField("name"); }} className="p-1.5 rounded text-gray-400 hover:text-emerald-800 hover:bg-emerald-50"><Pencil size={12} /></button>
                        <button onClick={() => setDelTarget({ id: l.id, type: "line", name: l.name })} className="p-1.5 rounded text-gray-400 hover:text-red-500 hover:bg-red-50"><Trash2 size={12} /></button>
                      </div>
                    )}
                  </td>
                </tr>
        ...
        {activeSection === "zones" && (
          <table className="w-full text-sm">
            ...
            {/* ["Zone Name", "Type", "Line", "Plant", "Default Lang", ""] */}
              {["Zone Name", "Type", "Line", "Plant", ""].map((h) => <th key={h} className="px-4 py-3 text-left text-[11px] font-semibold text-gray-600 uppercase tracking-wide">{h}</th>)}
            </tr></thead>
            <tbody className="divide-y divide-gray-50">
              {zones.map((z) => {
                ...
                return (
                  <tr key={z.id} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-3 text-xs text-gray-500">{z.type}</td>
                    <td className="px-4 py-3 text-xs text-gray-500">{line?.name || z.line_id}</td>
                    <td className="px-4 py-3 text-xs text-gray-500">{plant?.name || "—"}</td>
                    {/* <td className="px-4 py-3 text-xs">
                      {canEdit ? (
                        <select value={z.default_language || "EN"}
                          onChange={(e) => handleSaveEdit("zone", z.id, "default_language", e.target.value)}
                          className="border border-gray-200 rounded px-2 py-1 text-xs bg-white focus:outline-none focus:ring-1 focus:ring-zinc-400 text-gray-700">
                    ...
                    <td className="px-4 py-3">
                      {canEdit && (
                        <div className="flex gap-1 justify-end">
                          <button onClick={() => { setEditId(z.id); setEditField("name"); }} className="p-1.5 rounded text-gray-400 hover:text-emerald-800 hover:bg-emerald-50"><Pencil size={12} /></button>
                          <button onClick={() => setDelTarget({ id: z.id, type: "zone", name: z.name })} className="p-1.5 rounded text-gray-400 hover:text-red-500 hover:bg-red-50"><Trash2 size={12} /></button>
                        </div>
                      )}
                    </td>
                  </tr>
...
export default function DevicesZones() {
  ...
  if (loading) return <div className="flex justify-center py-20"><Loader2 className="h-8 w-8 animate-spin text-emerald-800" /></div>;
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex gap-2">
        <button onClick={() => setView("devices")}
          className={`px-4 py-2 rounded-lg text-sm font-semibold transition-colors ${view === "devices" ? "bg-emerald-800 text-white" : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"}`}>
          Devices
        </button>
        <button onClick={() => setView("structure")}
          className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold transition-colors ${view === "structure" ? "bg-emerald-800 text-white" : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"}`}>
      ...
      {view === "devices" && (
        <div className="flex gap-4 min-h-[560px]">
          {/* Left: Plant tree */}
          <div className="w-52 shrink-0 bg-white rounded-2xl border border-gray-200 shadow-sm overflow-y-auto">
            <div className="p-3 border-b border-gray-100">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Plant Tree</p>
            </div>
            <div className="p-2 space-y-0.5">
              <button type="button" onClick={() => { setSelectedPlant(null); setSelectedLine(null); setSelectedZone(null); }}
                className={`w-full text-left px-2 py-1.5 rounded text-xs font-medium transition-colors ${!selectedPlant && !selectedLine && !selectedZone ? "bg-emerald-50 text-emerald-800" : "text-gray-600 hover:bg-gray-50"}`}>
                All Plants
              </button>
              {plants.map((plant) => {
                ...
                return (
                  <div key={plant.id}>
                    <button type="button"
                      ...
                      className={`w-full text-left flex items-center gap-1 px-2 py-1.5 rounded text-xs font-semibold transition-colors ${selectedPlant === plant.id && !selectedLine ? "bg-emerald-50 text-emerald-800" : "text-gray-700 hover:bg-gray-50"}`}>
                      ...
                      return (
                        <div key={line.id} className="ml-3">
                          <button type="button"
                            ...
                            className={`w-full text-left flex items-center gap-1 px-2 py-1 rounded text-xs transition-colors ${selectedLine === line.id && !selectedZone ? "bg-emerald-50 text-emerald-800" : "text-gray-500 hover:bg-gray-50"}`}>
                          ...
                          {lineExpanded && lineZones.map((zone) => (
                            <button key={zone.id} type="button"
                              ...
                              className={`w-full text-left flex items-center gap-1 ml-2 px-2 py-1 rounded text-[11px] transition-colors ${selectedZone === zone.id ? "bg-emerald-100 text-emerald-800 font-semibold" : "text-gray-400 hover:bg-gray-50"}`}>
          ...
          {/* Center: Device table */}
          <div className="flex-1 min-w-0 bg-white rounded-2xl border border-gray-200 shadow-sm flex flex-col">
            <div className="p-4 border-b border-gray-100 flex items-center justify-between">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                {filteredDevices.length} device{filteredDevices.length !== 1 ? "s" : ""}
              </p>
              <div className="flex gap-2">
                <button type="button" onClick={() => refreshDevices()} className="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 transition-colors"><RefreshCw size={14} /></button>
                {canEdit && <button type="button" onClick={() => setAddWizardOpen(true)} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-800 text-white rounded-lg text-xs font-semibold hover:bg-emerald-800"><Plus size={13} /> Add Device</button>}
              </div>
            </div>
                ...
                <tbody className="divide-y divide-gray-50">
                  {filteredDevices.map((d) => (
                    <tr key={d.id} onClick={() => setSelectedDevice(d)}
                      className={`cursor-pointer hover:bg-gray-50/50 transition-colors ${selectedDevice?.id === d.id ? "bg-emerald-50/50" : ""}`}>
                      ...
                      <td className="px-4 py-3 text-gray-500 text-xs">{d.zone_name}</td>
                      <td className="px-4 py-3 font-mono text-xs text-gray-500">{d.address || d.ip || "—"}</td>
                      <td className="px-4 py-3"><HeartbeatTime ts={d.last_heartbeat} /></td>
                      <td className="px-4 py-3"><StatusPill status={d.status} /></td>
                      <td className="px-4 py-3">
                        <button type="button" onClick={(e) => { e.stopPropagation(); setSelectedDevice(d); }} className="p-1 rounded text-gray-400 hover:text-emerald-800" title="View details">
                          <Edit2 size={14} />
                        </button>
                      </td>
                    </tr>
          ...
          {/* Right: Device detail */}
          {selectedDevice && (
            <div className="w-80 shrink-0 bg-white rounded-2xl border border-gray-200 shadow-sm overflow-y-auto">
              <div className="p-4 border-b border-gray-100 flex items-center justify-between">
                <div>
                  <p className="font-semibold text-gray-800 text-sm">{selectedDevice.name}</p>
                  <p className="text-xs text-gray-400">{selectedDevice.type}</p>
                </div>
                <button type="button" onClick={() => setSelectedDevice(null)} className="text-gray-400 hover:text-gray-600 text-lg leading-none">×</button>
              </div>
              <div className="p-4 flex flex-col gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-sm text-gray-700 flex-1">
                      {selectedDevice.address || selectedDevice.ip || <span className="text-gray-400 italic">Not set</span>}
                    </span>
                    {canEdit && (
                      <button type="button"
                        onClick={() => { setEditIpValue(selectedDevice.address || selectedDevice.ip || ""); setEditIpOpen(true); }}
                        className="p-1 rounded text-gray-400 hover:text-emerald-800 hover:bg-emerald-50 transition-colors"
                ...
                <div>
                  ...
                  {!selectedDevice.address && !selectedDevice.ip ? (
                    <p className="text-xs text-gray-400 italic">No IP address configured</p>
                  ) : deviceStatusLoading ? (
                    ...
                    <div className="space-y-1 mt-1">
                      {Object.entries(deviceStatus).map(([k, v]) => (
                        <div key={k} className="flex justify-between text-xs">
                          ...
                          <span className="font-mono font-semibold text-gray-700 max-w-[140px] truncate" title={String(v)}>
                    ...
                    <p className="text-xs text-gray-400 italic">No status data</p>
                  )}
                </div>

                {selectedDevice.health && (
                  <div>
                    <p className={LABEL}>Node Health {selectedDevice.health_updated_at && <span className="normal-case font-normal text-gray-400">· {timeAgo(selectedDevice.health_updated_at)}</span>}</p>
                    <div className="space-y-1">
                      <div className="flex justify-between text-xs"><span className="text-gray-500">CPU Temperature</span><span className="font-mono font-semibold">{selectedDevice.health.cpu_temp_c != null ? `${selectedDevice.health.cpu_temp_c}°C` : "—"}</span></div>
                      <div className="flex justify-between text-xs"><span className="text-gray-500">Uptime</span><span className="font-mono font-semibold">{selectedDevice.health.uptime_sec != null ? formatDuration(Math.floor(selectedDevice.health.uptime_sec)) : "—"}</span></div>
                      <div className="flex justify-between text-xs"><span className="text-gray-500">Memory Used</span><span className="font-mono font-semibold">{selectedDevice.health.mem_percent != null ? `${selectedDevice.health.mem_percent}%` : "—"}</span></div>
                      <div className="flex justify-between text-xs"><span className="text-gray-500">Audio Queue</span><span className="font-mono font-semibold">{selectedDevice.health.queue_depth ?? 0}</span></div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {selectedDevice.type === "Gateway" && <>
                        <div className="flex justify-between text-xs"><span className="text-gray-500">CPU</span><span className="font-mono font-semibold">{selectedDevice.metrics.cpu ?? "—"}%</span></div>
                        <div className="flex justify-between text-xs"><span className="text-gray-500">Memory</span><span className="font-mono font-semibold">{selectedDevice.metrics.memory ?? "—"}%</span></div>
                        <div className="flex justify-between text-xs"><span className="text-gray-500">Latency</span><span className="font-mono font-semibold">{selectedDevice.metrics.latency_ms ?? "—"} ms</span></div>
                        <div className="flex justify-between text-xs"><span className="text-gray-500">Audio Queue</span><span className="font-mono font-semibold">{selectedDevice.metrics.audio_queue ?? 0}</span></div>
                      </>}
                      {selectedDevice.type === "Speaker" && <>
                        <div className="flex justify-between text-xs"><span className="text-gray-500">SPL</span><span className="font-mono font-semibold">{selectedDevice.metrics.spl_db ?? "—"} dB</span></div>
                        <div className="flex justify-between text-xs"><span className="text-gray-500">Impedance</span><span className="font-mono font-semibold">{selectedDevice.metrics.impedance_ohm ?? "—"} Ω</span></div>
                      </>}
                    </div>
                  </div>
                )}
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-2">
                    <button type="button"
                      ...
                      className="w-full flex items-center gap-2 px-3 py-2 border border-emerald-200 text-emerald-800 rounded-lg text-xs font-medium hover:bg-emerald-50 transition-colors disabled:opacity-50">
      ...
      {editIpOpen && selectedDevice && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6 z-10 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-gray-900 text-sm">Edit Device Details</h3>
              <button type="button" onClick={() => setEditIpOpen(false)} className="text-gray-400 hover:text-gray-600 text-xl leading-none">×</button>
            </div>
            <div>
              <p className="text-xs text-gray-500 mb-1">Device: <span className="font-medium text-gray-700">{selectedDevice.name}</span></p>
            </div>
            <div>
            ...
            <div className="flex justify-end gap-2 pt-1 border-t border-gray-100">
              ...
              <button type="button" onClick={handleSaveIp} disabled={savingIp || !editIpValue.trim()}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-800 text-white rounded-lg text-sm font-semibold hover:bg-emerald-800 disabled:opacity-50">
      ...
      {addWizardOpen && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 p-6 z-10 flex flex-col gap-5">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-gray-900">Add Device — Step {wizardStep}/3</h3>
              <button type="button" onClick={() => { setAddWizardOpen(false); setWizardStep(1); }} className="text-gray-400 hover:text-gray-600 text-xl leading-none">×</button>
            </div>
            {wizardStep === 1 && (
              <div className="flex flex-col gap-4">
            ...
            {wizardStep === 2 && (
              <div className="flex flex-col gap-4">
                <p className="text-sm text-gray-500">Assign this device to a zone.</p>
            ...
            {wizardStep === 3 && (
              <div className="flex flex-col gap-3">
                ...
                <div className="bg-gray-50 rounded-lg p-4 text-sm space-y-1">
                  <div className="flex justify-between"><span className="text-gray-500">Name:</span><span className="font-medium">{newDevice.name}</span></div>
                  <div className="flex justify-between"><span className="text-gray-500">Type:</span><span className="font-medium">{newDevice.type}</span></div>
                  <div className="flex justify-between"><span className="text-gray-500">IP:</span><span className="font-mono">{newDevice.ip}</span></div>
                  <div className="flex justify-between"><span className="text-gray-500">Zone:</span><span className="font-medium">{zones.find((z) => z.id === newDevice.zone_id)?.name ?? "—"}</span></div>
                </div>
              </div>
            )}
            <div className="flex justify-between pt-2 border-t border-gray-100">
              <button type="button" onClick={() => wizardStep > 1 ? setWizardStep((s) => s - 1) : setAddWizardOpen(false)} className="px-4 py-2 border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-50">
                {wizardStep === 1 ? "Cancel" : "Back"}
              </button>
              {wizardStep < 3
                ? <button type="button" onClick={() => setWizardStep((s) => s + 1)} disabled={wizardStep === 1 && !newDevice.name} className="px-4 py-2 bg-emerald-800 text-white rounded-lg text-sm font-semibold hover:bg-emerald-800 disabled:opacity-50">Next</button>
                : <button type="button" onClick={handleAddDevice} className="px-4 py-2 bg-emerald-800 text-white rounded-lg text-sm font-semibold hover:bg-emerald-800">Save Device</button>
              }
            </div>
          </div>
        </div>
