    ...
    <div className="flex flex-col gap-4">
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 flex items-center justify-between">
        <div className="flex items-center gap-2 text-gray-600">
          <Megaphone size={16} className="text-emerald-800" aria-hidden="true" />
          <span className="text-sm font-semibold">Manual Broadcast</span>
          <span className="text-xs text-gray-400">— broadcast a message to zone speakers now</span>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 flex flex-col gap-4">
        ...
        <div>
          <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1 block">Message Type</label>
          <div className="flex gap-3">
            {[{ v: "text", l: "Type message" }, { v: "clip", l: "Use pre-recorded clip" }].map(({ v, l }) => (
              <label key={v} className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name="bcast-mode" value={v} checked={broadcastMode === v} onChange={() => setBroadcastMode(v)} className="text-emerald-800" />
        ...
        {broadcastMode === "text" ? (
          <div>
            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1 block">Message Text</label>
            <textarea
              rows={3}
              value={broadcastMsg}
              onChange={(e) => setBroadcastMsg(e.target.value)}
              placeholder="Type the message to broadcast…"
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-1 focus:ring-zinc-400 focus:border-zinc-400 text-gray-700 resize-none"
              aria-label="Broadcast message text"
            />
          </div>
        ) : (
          <div className="flex flex-col gap-2">
            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1 block">Audio Clip</label>
            <div className="flex items-center gap-2">
              <select
                value={broadcastClipId}
                onChange={(e) => setBroadcastClipId(e.target.value)}
                className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-1 focus:ring-zinc-400 text-gray-700"
        ...
        <div className="flex justify-end gap-3 pt-2 border-t border-gray-100">
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-2 bg-emerald-800 hover:bg-emerald-900 text-white rounded-lg text-sm font-semibold disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:ring-offset-1"
          >
