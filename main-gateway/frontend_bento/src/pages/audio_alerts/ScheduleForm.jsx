...
const INPUT = "w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-gray-700";
const LABEL = "text-sm font-semibold text-gray-600 mb-1.5 block";
const SECTION = "bg-white rounded-2xl border border-gray-200 shadow-sm p-5 flex flex-col gap-4";
const SECTION_TITLE = "text-base font-bold text-gray-800";
...
function OptionCard({ active, onClick, icon: Icon, label, sub }) {
  return (
    <button
      ...
      aria-pressed={active}
    >
      <Icon size={20} className={active ? "text-emerald-800" : "text-gray-400"} aria-hidden="true" />
      <span className={`text-sm font-bold ${active ? "text-emerald-800" : "text-gray-700"}`}>{label}</span>
      <span className="text-xs text-gray-400 leading-snug">{sub}</span>
    </button>
  );
}
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-3">
        <button type="button" onClick={onCancel} className="p-2 rounded-lg hover:bg-gray-100 text-gray-500" aria-label="Back to schedules">
          <ArrowLeft size={18} aria-hidden="true" />
        </button>
        <h2 className="text-lg font-bold text-gray-800">{initialSchedule ? "Edit Announcement" : "New Scheduled Announcement"}</h2>
      </div>
      ...
      <div className={SECTION}>
        <button
          ...
          className={`flex items-center justify-between gap-3 rounded-xl border-2 p-4 text-left transition-colors ${
            form.is_enabled ? "border-emerald-400 bg-emerald-50" : "border-gray-200 bg-gray-50"
          }`}
        >
          <span className="flex items-center gap-3">
            <Power size={22} className={form.is_enabled ? "text-emerald-600" : "text-gray-400"} aria-hidden="true" />
            <span>
              <span className="block text-base font-bold text-gray-800">Enable this announcement</span>
              <span className={`block text-sm font-semibold ${form.is_enabled ? "text-emerald-700" : "text-gray-500"}`}>
                {form.is_enabled ? "Enabled — will play as scheduled" : "Disabled — will not play"}
              </span>
            </span>
          </span>
          <span className={`shrink-0 w-14 h-8 rounded-full flex items-center px-1 transition-colors ${form.is_enabled ? "bg-emerald-500 justify-end" : "bg-gray-300 justify-start"}`}>
          ...
          <div className="flex gap-3 mb-2">
            {[{ v: "text", l: "Type a message" }, { v: "clip", l: "Use a recorded clip" }].map(({ v, l }) => (
              <label key={v} className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name="sched-mode" value={v} checked={form.audio_mode === v} onChange={() => set("audio_mode", v)} className="text-emerald-800" />
            ...
            <div className="flex flex-col gap-2">
              ...
              <div>
                <button type="button" onClick={() => clipFileRef.current?.click()} disabled={clipUploading}
                  className="inline-flex items-center gap-1.5 text-xs font-medium text-emerald-800 hover:text-emerald-800 disabled:opacity-50">
      ...
      <div className={SECTION}>
            ...
            <div className="flex flex-wrap gap-2">
              {QUICK_PRESETS.map((p) => (
                <button key={p.minutes} type="button" onClick={() => setQuickPreset(p.minutes)}
                  className="px-4 py-2.5 rounded-lg border-2 border-gray-200 hover:border-emerald-500 text-sm font-semibold text-gray-700">
                  {p.label}
                </button>
              ))}
            </div>
        ...
        {form.whenMode === "hourly" && (
          <div className="flex flex-wrap items-end gap-6 pt-2 border-t border-gray-100">
            <div>
        ...
        {form.whenMode === "shift" && (
          <div className="flex flex-col gap-4 pt-2 border-t border-gray-100">
            {shiftNames.length === 0 ? (
              <p className="text-sm text-gray-400">No shifts are configured yet. Set them up in App Settings → Shift Times.</p>
            ) : (
              <>
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {shiftNames.map((name) => (
                      <button key={name} type="button" onClick={() => set("shift_name", name)}
                        className={`px-4 py-2.5 rounded-lg border-2 text-sm font-semibold ${form.shift_name === name ? "border-emerald-600 bg-emerald-50 text-emerald-800" : "border-gray-200 text-gray-600"}`}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[
                      ...
                      <button key={opt.v} type="button" onClick={() => set("shift_event", opt.v)}
                        className={`px-4 py-2.5 rounded-lg border-2 text-sm font-semibold ${form.shift_event === opt.v ? "border-emerald-600 bg-emerald-50 text-emerald-800" : "border-gray-200 text-gray-600"}`}>
                        {opt.l}
                      </button>
                    ))}
                  </div>
                      ...
                      <div className="flex gap-2">
                        {[{ v: true, l: "Before start" }, { v: false, l: "After start" }].map((opt) => (
                          <button key={String(opt.v)} type="button" onClick={() => set("shift_offset_before", opt.v)}
                            className={`px-4 py-2.5 rounded-lg border-2 text-sm font-semibold ${form.shift_offset_before === opt.v ? "border-emerald-600 bg-emerald-50 text-emerald-800" : "border-gray-200 text-gray-600"}`}>
                            {opt.l}
                          </button>
                        ))}
                      </div>
              ...
              <div className="flex gap-2">
                {[{ v: false, l: "Every day" }, { v: true, l: "Working days only" }].map((opt) => (
                  <button key={String(opt.v)} type="button" onClick={() => set("dailyWorkingDaysOnly", opt.v)}
                    className={`px-4 py-2.5 rounded-lg border-2 text-sm font-semibold ${form.dailyWorkingDaysOnly === opt.v ? "border-emerald-600 bg-emerald-50 text-emerald-800" : "border-gray-200 text-gray-600"}`}>
                    {opt.l}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5 mb-2">
                <button type="button" onClick={() => set("days_of_week", [0, 1, 2, 3, 4, 5, 6])}
                  className="px-3 py-1.5 rounded-full text-xs font-semibold border-2 border-gray-200 text-gray-500 hover:border-emerald-300">
                  Every day
                </button>
                ...
                {DAYS.map((d) => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3.5 py-2 rounded-full text-sm font-semibold border-2 transition-all ${active ? "bg-emerald-100 text-emerald-800 border-emerald-500" : "bg-gray-50 text-gray-500 border-gray-200"}`}
                    >
                      {d.l}
                    </button>
                  );
      ...
      <div className={SECTION}>
        ...
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" checked={form.plant_wide} onChange={(e) => set("plant_wide", e.target.checked)} className="w-4 h-4 rounded border-gray-300 text-emerald-800" />
          <span className="text-sm text-gray-700 font-medium">Everywhere (plant-wide)</span>
        </label>
        {!form.plant_wide && (
      ...
      <div className="rounded-xl border-2 border-emerald-200 bg-emerald-50 p-5">
        <p className="text-[11px] font-bold text-emerald-600 uppercase tracking-widest mb-1.5">Before you save</p>
        <p className="text-base font-semibold text-emerald-900">{summaryText}</p>
      </div>

      {/* 6. Save */}
      <div className="flex justify-end gap-3">
        <button type="button" onClick={onCancel} className="px-5 py-2.5 border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50">
          Cancel
        </button>
        <button
          type="button" onClick={handleSubmit} disabled={saving}
          className="inline-flex items-center gap-2 px-6 py-2.5 bg-emerald-800 hover:bg-emerald-800 text-white rounded-lg text-sm font-bold disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:ring-offset-1"
        >
